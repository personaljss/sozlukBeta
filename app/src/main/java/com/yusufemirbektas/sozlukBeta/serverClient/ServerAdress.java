package com.yusufemirbektas.sozlukBeta.serverClient;

public class ServerAdress {
    //server base url
    public static final String SERVER_URL = "https://hostingdenemesi.online/";
    //login related
    public static final String REGISTER = "loginfiles/registerprocess/";
    public static final String LOGIN = "loginfiles/loginprocess/";
    public static final String AUTO_LOGIN = "loginfiles/autologin/";
    public static final String ACTIVATION = "loginfiles/activationprocess/";
    //profile related
    public static final String PROFILE_PHP = "forumfiles/showprofile/";
    public static final String DISPLAY_PP = "generalfiles/uploadimages/";
    public static final String UPLOAD_PP = "generalfiles/downloadimages/";
    public static final String NEW_NICKNAME="loginfiles/newnickname/";
    public static final String SHOW_CHANNEL_SUBJECTS_PHP = "forumfiles/showchannelsubjects/";
    public static final String SHOW_SUBJECT_ENTRIES_PHP = "forumfiles/showsubjectentries/";
    public static final String CREATE_SUBJECT = "forumfiles/createsubject/";
    public static final String NEW_COMMENT = "forumfiles/newcomment/";
    public static final String LIKE_COMMENT = "forumfiles/likecomment/";
    public static final String SHOW_LIKE_DETAILS = "forumfiles/showlikedetails/";
    public static final String MAIN_FEED = "forumfiles/showfeed/";
    public static final String FOLLOW_USER = "forumfiles/followuser/";
    public static final String FOLLOWERS = "forumfiles/showfollowdetails/";
}